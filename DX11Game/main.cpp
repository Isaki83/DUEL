...
#define WINDOW_NAME		_T("オリジナルゲーム")		// ウインドウのキャプション名
...
HRESULT Init(HWND hWnd, BOOL bWindow)
{
	...
	// シーン初期化
	if (!CScene::InitAll()) { return E_FAIL; }
...
void Uninit(void)
{
	...
	// 深度ステンシルステート解放
	for (int i = 0; i < _countof(g_pDSS); ++i) { SAFE_RELEASE(g_pDSS[i]); }

	// ブレンド ステート解放
	for (int i = 0; i < MAX_BLENDSTATE; ++i) { SAFE_RELEASE(g_pBlendState[i]); }

	// ラスタライザ ステート解放
	for (int i = 0; i < MAX_CULLMODE; ++i) { SAFE_RELEASE(g_pRs[i]); }
...
{
	if (nCullMode >= 0 && nCullMode < MAX_CULLMODE) { g_pDeviceContext->RSSetState(g_pRs[nCullMode]); }
}
