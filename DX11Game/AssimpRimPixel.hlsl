// Assimp用ピクセルシェーダ (AssimpRimPixel.hlsl)
// リムライト
...
float4 main(VS_OUTPUT input) : SV_Target0
{
	...
	// *** リムライト ***
	// 縁部分の計算
	float3 edgeColor = g_RimColor.rgb/*float3(0.0f, 0.0f, 1.0f)*/;	// 縁部分に色を持たせる
	float edge = min(1.0f - dot(N, V), 1.0f);
	edge = pow(edge, 2);							// 縁部分が満遍なく広がらないように絞る
