// Assimpライブラリ用モデルクラス [AssimpModel.h]
#pragma once
#define NOMINMAX
//#define D3DCOMPILER
#include <map>
...
struct SceneAnimNode 
{
	...
	~SceneAnimNode()
	{
		for (std::vector<SceneAnimNode*>::iterator it = mChildren.begin(); it != mChildren.end(); ++it) { delete *it; }
	}
};
...
private:
	...
	static ID3D11InputLayout* m_pVertexLayout[2];
	static ID3D11VertexShader* m_pVertexShader[2];
	static ID3D11PixelShader* m_pPixelShader[2];
	static ID3D11SamplerState* m_pSampleLinear[2];
...
public:
	...
	void Draw(ID3D11DeviceContext* pDC, DirectX::XMFLOAT4X4& mtxWorld, EByOpacity byOpacity = eNoAffect, int ShaderNumber = 0);
	void DrawNode(ID3D11DeviceContext* pDC, aiNode* piNode, const aiMatrix4x4& piMatrix, EByOpacity byOpacity, int ShaderNumber = 0);
	DirectX::XMFLOAT4X4& GetWorldMatrix() { return m_mtxWorld; }									// ShaderNumber
	void SetMaterial(TAssimpMaterial* pMaterial = nullptr) { m_pMaterial = pMaterial; }				// 0 : 普通
	TAssimpMaterial* GetMaterial() { return m_pMaterial; }											// 1 : リムライト
	...
	DirectX::XMFLOAT3& GetBBox(float scale = 1.0f) { return m_vBBox; }
