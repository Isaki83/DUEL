	...
	_Out_opt_ TexMetadata* pTexInfo)
{
	*textureView = nullptr;
	...
	HRESULT hr;
	if (_wcsicmp(wszExt, L".tga") == 0) { hr = LoadFromTGAFile(szFileName, &meta, image); }
	else if (_wcsicmp(wszExt, L".dds") == 0) { hr = LoadFromDDSFile(szFileName, DDS_FLAGS_FORCE_RGB, &meta, image); }
	else { hr = LoadFromWICFile(szFileName, WIC_FLAGS_FORCE_RGB, &meta, image); }
	if (FAILED(hr)) { return hr; }
	if (pTexInfo) { *pTexInfo = meta; }
	return CreateShaderResourceView(d3dDevice, image.GetImages(), image.GetImageCount(), meta, textureView);
}
