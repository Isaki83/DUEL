...
void CSound::Init(void)
{
	...
	if (!m_pXAudio2) 
	{
		...
		if (FAILED(hr)) { m_pXAudio2 = nullptr; }
		else 
		{
#if defined(_DEBUG)
...
void CSound::Update(void)
{
	if (m_pBgm) 
	{
		for (int i = 0; i < MAX_BGM; ++i) { m_pBgm[i].Update(); }
	}
}
...
void CSound::Fin(void)
{
	if (m_pBgm) 
	{
		for (int i = 0; i < MAX_BGM; ++i) { m_pBgm[i].Close(); }
	}
	if (m_pSe) 
	{
		for (int i = 0; i < MAX_SE; ++i) { m_pSe[i].Close(); }
	}
	MFShutdown();
...
void CSound::Pause()
{
	for (int i = 0; i < MAX_SE; ++i) { m_pSe[i].Pause(); }
	for (int i = 0; i < MAX_BGM; ++i) { m_pBgm[i].Pause(); }
}
...
void CSound::Resume()
{
	for (int i = 0; i < MAX_SE; ++i) { m_pSe[i].Resume(); }
	for (int i = 0; i < MAX_BGM; ++i) { m_pBgm[i].Resume(); }
}
...
{
	if (!m_pBgm || bgm < 0 || bgm >= MAX_BGM) { return 0.0f; }
...
{
	if (!m_pSe || se < 0 || se >= MAX_SE) { return 0.0f; }
...
CSoundStream::CSoundStream() :
	...
	m_status(SS_STOP)
{
	for (int i = 0; i < MAX_BUFFER_COUNT; ++i) { m_buffers[i].resize(32768); }
}
...
{
	for (int i = 0; i < MAX_DUP; ++i) { SAFE_DESTROYVOICE(m_pSourceVoice[i]); }
}
...
namespace {
	HRESULT CreateMFReader(LPCWSTR mediaFile, COMPTR(IMFSourceReader)& reader, WAVEFORMATEX* wfx, size_t maxwfx)
	{
		if (!mediaFile || !wfx) { return E_INVALIDARG; }
